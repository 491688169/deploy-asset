...
function _createUploader(opts, DEFAULTS) {
  ...
  if (!(uploader instanceof Uploader)) uploader = Uploader.instance(opts.uploaderName, opts.uploaderOpts, opts.env);
...
function _configOptsAccordingUploader(opts, uploader) {
  ...
  if (uploader.retrieveRemoteUrlAfterUploaded && opts.concurrence !== 1) {
    ylog.warn(`${opts.uploaderName}上传器需要在文件上传完后才能返回文件链接，` + `所以无法异步上传，自动设置 ~opts.concurrence = ${max}~ `);
    opts.concurrence = 1;
  }
}
...
export default function (any, opts, next) {
  ...
  try {
    ...
    if (!rootDir) rootDir = typeof any === 'string' && path.isDirectorySync(any) ? any : util.getFilesCommonDirectory(filePaths);
